...
void showTree(Dendrogram d) {
    void    doShowTree(Dendrogram);
    doShowTree(d);
}
...
asciinode *build_ascii_tree(Dendrogram d);
...
void doShowTree(Dendrogram d)
{
  asciinode *proot;
  int xmin, i;
  if (d == NULL) return;
  proot = build_ascii_tree(d);
...
asciinode *build_ascii_tree_recursive(Dendrogram d)
{
  asciinode * node;

  if (d == NULL) return NULL;
  node = malloc(sizeof(asciinode));
  node->left = build_ascii_tree_recursive(d->left);
  node->right = build_ascii_tree_recursive(d->right);
  if (node->left != NULL) node->left->parent_dir = -1;
  if (node->right != NULL) node->right->parent_dir = 1;
  sprintf(node->label, "%d", d->vertex);
...
asciinode *build_ascii_tree(Dendrogram d)
{
  asciinode *node;
  if (d == NULL) return NULL;
  node = build_ascii_tree_recursive(d);
  node->parent_dir = 0;
  return node;
}
...
int main(void) {
  ...
  // showGraph(g);
  Dendrogram d = LanceWilliamsHAC(g, 2);

  // printing function taken from lab10
  showTree(d);
